...
task axi4_write_address_channel_task (inout axi4_write_transfer_char_s data_write_packet, axi4_transfer_cfg_s cfg_packet);
  ...
  task axi4_read_data_channel_task (inout axi4_read_transfer_char_s data_read_packet, input axi4_transfer_cfg_s cfg_packet);
    ...
    `uvm_info(name,$sformatf("READ DATA CHANNEL: waiting for rvalid..................."),UVM_MEDIUM)
    do begin
      @(posedge aclk);
    ...
    `uvm_info(name,$sformatf("READ DATA CHANNEL: detected rvalid ......................"),UVM_MEDIUM)

    repeat(data_read_packet.no_of_wait_states)begin
      `uvm_info(name,$sformatf("DRIVING WAIT STATES in read data channel :: %0d",data_read_packet.no_of_wait_states),UVM_NONE);
      @(posedge aclk);
    end
   ...
   // rready <= 1'b1;
    rready <= 1'b1;
      ...
      `uvm_info(name,$sformatf("DRIVING WAITAAAAAAAAAAAAA in read data channel :: %0d",data_read_packet.no_of_wait_states),UVM_NONE);
    forever begin
      do begin
        @(posedge aclk);
      end while(rvalid === 1'b0);
   ...
   // @(posedge aclk); // !!!! syn deassert with rvalid
    rready <= 1'b0;
